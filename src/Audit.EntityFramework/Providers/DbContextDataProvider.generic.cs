...
{
    /// <summary>
    /// Generic Audit Data Provider using Entity Framework Core for storing Audit Events.
    /// </summary>
    /// <remarks>
    /// <para>
    /// Use this data provider when all audit events are stored in a single entity or table.
    /// Use the alternative non-generic <see cref="DbContextDataProvider"/> for scenarios where audit events are distributed across different entities or tables and no event replacement (update) is needed.
    /// </para>
    /// <para>
    /// This data provider allow replacing events when the CreationPolicy is set to <see cref="EventCreationPolicy.InsertOnStartReplaceOnEnd"/>. 
    /// </para>
    /// </remarks>
