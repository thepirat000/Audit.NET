...
> **Note**
> 
> The `DbContextDataProvider` differs from the `EntityFrameworkDataProvider` in that it is a 
generic provider that can be used to store any kind of audit event, not just the high-level 
events generated by the Entity Framework Core SaveChanges interceptor.
