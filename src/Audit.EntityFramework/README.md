...
- **IncludeIndependantAssociations**: Value to indicate if the Independant Associations should be included. (Only for EF <=5, since EF Core does not support many-to-many relations without a join entity. Check [this](https://github.com/aspnet/EntityFrameworkCore/issues/1368)).
...
| **Associations** | Array of [AssociationEntry](#associationentry) | Independant associations changes, many-to-many relations without a join table with changes (only for EF <=5, not available on EF Core) |
